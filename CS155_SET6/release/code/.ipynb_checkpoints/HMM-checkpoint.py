...
class HiddenMarkovModel:
    ...
    def viterbi(self, x):
        ...
        for state in range(self.L):
            ...
            probs[0][state] = np.log(self.A_start) + np.log(self.O[state][x[0]])
...
#                                                 y_prev) for y_prev in range(L))
                ...
                probs[prev_x+1][state] = cur_prob
                seqs[prev_x+1][state] = seqs[prev_x][append_state] + str(state)
                
        (prob, state) = max((probs[M][state], y) for y in range(L))
